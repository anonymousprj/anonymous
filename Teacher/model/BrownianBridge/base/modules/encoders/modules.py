...
class ATTMAP_conditioning(nn.Module):
    def __init__(self):
        super().__init__()

        
        self.ct_down = nn.Sequential(
                            nn.Conv3d(1, 128, kernel_size=3, stride=1, padding=1), 
        ...
        self.ff = FeedForward(dim=128, hidden_dim=512, dropout=0.1)
    def forward(self, x_cond_latent, attmap):
        y = self.ct_down(attmap) 
        ...
        x_attn = x_attn + x
        x_attn = self.ff(x_attn) + x_attn

        x_attn = x_attn.permute(0, 2, 1)
        x_attn = x_attn.view(x_attn.shape[0], x_attn.shape[1], 4, 4)
        x_attn = self.deconv(x_attn)
